...
public class QuickImpl extends AbstractQuick implements ICompare{
    ...
    public List sort(double[] doubles, OrderEnum orderEnum) throws Throwable {
        if(OrderEnum.asc.equals(orderEnum)){
            quickSortByAsc(doubles,0,doubles.length-1);
        }else {
            quickSortByDes(doubles,0,doubles.length-1);
        }
     ...
     * 升序排序
     * **/
    private void quickSortByAsc(double[] doubles, int start, int end){
        if(start >= end){
            return;
        }
         ...
         * **/
        //比原基准值小一个数为需要排序的分组
        quickSortByAsc(doubles,start,i-1);
        //比原基准值大一个数为需要排序的分组
        quickSortByAsc(doubles,i+1,end);
    }
    /**
     * 快排 分治法：从前往后 && 从后往前
     * 降序排序
     * **/
    private void quickSortByDes(double[] doubles, int start, int end){
        if(start >= end){
            return;
        }
        //基准值(一般取第一个或最后一个)：以常量定义，表示方法内执行时不可变
         ...
         * **/
        //比原基准值小一个数为需要排序的分组
        quickSortByDes(doubles,start,i-1);
        //比原基准值大一个数为需要排序的分组
        quickSortByDes(doubles,i+1,end);
    }
